			    ...
			    int insn_flags)
{
	...
	if (cur->insn_hist_end + 1 > env->insn_hist_cap) {
		...
		prev_size = size_mul(env->insn_hist_cap, sizeof(*p));
		p = kvrealloc(env->insn_hist, prev_size, alloc_size, GFP_USER);
		if (!p)
			return -ENOMEM;
		env->insn_hist = p;
...
process_func:
	 ...
	 */
	subprog_depth = round_up(max_t(u32, subprog[idx].stack_depth, 1), 32);
	if (priv_stack_supported) {
			     ...
			     struct bpf_insn *insn, int *insn_idx)
{
	...
	if (insn_flags) {
		err = push_insn_history(env, this_branch, insn_flags);
		if (err)
			return err;
	}
...
static int convert_ctx_accesses(struct bpf_verifier_env *env)
{
	...
	int i, cnt, size, ctx_field_size, ret, delta = 0;
	const int insn_cnt = env->prog->len;
