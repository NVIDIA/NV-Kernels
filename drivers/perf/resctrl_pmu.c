...
static int resctrl_pmu_event_init(struct perf_event *event)
{
	...
	cpus_read_lock();
	d = resctrl_find_domain(&r->mon_domains, domain_id, NULL);
...
static void resctrl_pmu_event_update(struct perf_event *event)
{
	...
	m = container_of(d, struct rdt_mon_domain, hdr);

	do {
		prev = local64_read(&hwc->prev_count);

		err = resctrl_arch_rmid_read(r, m, closid, rmid,
					     event_num, &now,
					     event->pmu_private);
		if (err)
			return;
