				   ...
				   void **out_cfg)
{
	...
	/*
	 * Device configs are stored under padded ACPI paths (\_SB_.USB0);
	 * accept the unpadded spelling users type (\_SB.USB0) too.
	 */
	if (pwrap_acpi_normalize_path(path, norm, sizeof(norm)))
		return -EINVAL;

	*out_cfg = pwrap_query_dev_config(norm);
	if (!*out_cfg) {
							...
							const char *buf, size_t count)
{
	...
	/* The config table holds padded paths (\_SB_.USB0); accept the unpadded form too. */
	if (pwrap_acpi_normalize_path(acpi_path, norm, sizeof(norm)))
		return -EINVAL;

	dev_cfg = mtk_pwrap_dev_probe(norm);
	if (!dev_cfg) {
							...
							const char *buf, size_t count)
{
	...
	/* Store the padded form so the lookup in _show() matches the table. */
	if (pwrap_acpi_normalize_path(raw, search_path, sizeof(search_path))) {
		search_path[0] = '\0';
		mutex_unlock(&search_path_lock);
		return -EINVAL;
	}
 ...
 */
/*
 * acpi_get_first_physical_node() drops adev->physical_node_lock before it
 * returns, so the struct device it points to can be unbound from the ACPI
 * node and released before the caller gets to take a reference. Walk the
 * physical node list here instead and take the reference while the lock is
 * held, so the pointer handed back is always one the caller owns.
 */
static struct device *pwrap_get_first_physical_node(struct acpi_device *adev)
{
	struct acpi_device_physical_node *node;
	struct device *dev = NULL;

	mutex_lock(&adev->physical_node_lock);
	node = list_first_entry_or_null(&adev->physical_node_list,
					struct acpi_device_physical_node, node);
	if (node)
		dev = get_device(node->dev);
						  ...
						  const char *buf, size_t count)
{
	 ...
	 *
	 * The reference on the physical device is taken under the ACPI
	 * device's physical_node_lock (see pwrap_get_first_physical_node())
	 * before the acpi_device reference acquired by acpi_get_acpi_dev() is
	 * dropped.
	 */
	phys_dev = pwrap_get_first_physical_node(adev);
	acpi_dev_put(adev);
