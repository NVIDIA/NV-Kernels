...
/* Last mailbox's doorbell words (MBOX_IN_SET/MBOX_OUT_CLR) must fit the window. */
#define SSPM_MBOX_MMIO_SPAN	(((SSPM_MBOX_NUM - 1) * 0x10000) + 0x1008)
...
/*
 * SET completion state, all under _hw_rsc except for the handler's reads
 * and its single store to _hw_ack.
 *
 * _inflight_hdr is the SCMI header of the command currently waiting for
 * its ack (0 while nothing is in flight or after a timeout, which no reply
 * header can equal). The SET mailbox handler reads the reply header back
 * from the slot and completes the wait only when message id, protocol and
 * token match it, so a late ack from a timed-out transaction cannot be
 * consumed by the next command.
 */
static int _hw_ack;
static u32 _inflight_hdr;
/* Set once a reply echoed our token; until then a token-0 echo is accepted. */
static bool _token_echo_seen;
static unsigned int _stale_acks;
/*
 * Set by a SET timeout: the next sspm_ci_set() first gives the late reply a
 * bounded window to arrive (and be dropped as stale) before it rewrites the
 * shared slot and publishes a new header. Without this, a firmware that does
 * not echo tokens could have that late reply complete the next command, and
 * a late interrupt could read the next command's header back from the slot.
 */
static bool _drain_needed;
#define SSPM_CI_DRAIN_MS	100
 ...
 * indistinguishable). Packed as a u8 into the header token field. Token 0
 * is never sent: a reply carrying token 0 is then unambiguously a firmware
 * that does not echo the sender's token, see sspm_ci_ack_matches().
 */
static u8 _scmi_token = 1;

static u8 sspm_ci_next_token(void)
{
	u8 token = _scmi_token++;
...
/*
 * After a timed-out SET, wait a bounded time for its late reply so the
 * handler drops it as stale (nothing is in flight) before the slot is reused.
 * Called with _hw_rsc held. Returns early as soon as a stale ack was seen.
 */
static void sspm_ci_drain_late_ack(void)
{
	unsigned int stale = READ_ONCE(_stale_acks);
	unsigned int ms;
	...
	for (ms = 0; ms < SSPM_CI_DRAIN_MS; ms++) {
		if (READ_ONCE(_stale_acks) != stale) {
			dev_dbg(__dev, "late SET ack drained after %u ms\n", ms);
			return;
		}
		mdelay(1);
	}
	...
	u32 p1, u32 p2, u32 p3, u32 p4, u32 p5)
{
	 ...
	 * trigger; the handler then matches the echoed reply header against
	 * _inflight_hdr, so a stale ack cannot complete this wait.
	 */
	WRITE_ONCE(_hw_ack, 0);
	/*
	 * Publish the header the handler must see echoed before ringing the
	 * doorbell, so the completion is tied to this transaction only.
	 */
	WRITE_ONCE(_inflight_hdr, message_header);
	...
	while (READ_ONCE(_hw_ack) == 0) {
		...
		if (count > SSPM_CI_MAX_POLL_COUNT) {
			/*
			 * Nothing in flight any more: a reply that arrives late
			 * for this header is dropped by the handler as stale
...
#define SCMI_MESSAGE_HEADER_CORRELATE_MASK \
	(SCMI_MESSAGE_HEADER_MESSAGE_ID_MASK | \
	 SCMI_MESSAGE_HEADER_PROTOCOL_ID_MASK | \
	 SCMI_MESSAGE_HEADER_TOKEN_MASK)

/* Reply header lives at the same slot offset as the command header. */
#define MBOX_MSG_HDR_OFS	(offsetof(struct scmi_msg_t, message_header))

/*
 * Decide whether the reply header read back from the SET mailbox completes
 * the command in flight. Message id, protocol and token must all match.
 * Firmware that does not echo the token (legacy, always replies with token
 * 0) is tolerated until the first real token echo is seen; after that a
 * mismatch is a stale ack. Because the sender never uses token 0, only a
 * match on a nonzero token counts as evidence of echo: a token-0 reply can
 * never take the exact-match branch, so a no-echo firmware stays on the
 * compatibility path for every transaction instead of only the first.
 */
static bool sspm_ci_ack_matches(u32 reply_hdr, u32 expect_hdr)
{
	u32 m = SCMI_MESSAGE_HEADER_CORRELATE_MASK;

	if (!expect_hdr)
		return false;
	if ((reply_hdr & m) == (expect_hdr & m)) {
		if (expect_hdr & SCMI_MESSAGE_HEADER_TOKEN_MASK)
			_token_echo_seen = true;
		return true;
	}
	if (!_token_echo_seen &&
	    !(reply_hdr & SCMI_MESSAGE_HEADER_TOKEN_MASK) &&
	    (reply_hdr & (m & ~SCMI_MESSAGE_HEADER_TOKEN_MASK)) ==
	    (expect_hdr & (m & ~SCMI_MESSAGE_HEADER_TOKEN_MASK))) {
		dev_info_once(__dev,
			      "firmware does not echo the SCMI token; ack correlation limited to message id and protocol\n");
		return true;
	}
	return false;
}
...
static irqreturn_t _mbox_handler_(int irq, void *data)
{
	...
	u32 reply_hdr = 0, expect_hdr = 0;

	if (id == SSPM_CI_SET_ID) {
		/* Read the echoed header before the slot is handed back. */
		reply_hdr = readl((void __iomem *)MBOX_BOX(id, MBOX_MSG_HDR_OFS));
		expect_hdr = READ_ONCE(_inflight_hdr);
	}

	writel(SSPM_CI_MAGIC, (void __iomem *)MBOX_OUT_CLR(id));
	/*
	 * The word at +0x4 is the mailbox ownership (mem_free) flag of the
	 * vendor transport, not an SCMI return status: TINYSYS SET carries no
	 * status word and the reference driver clears this flag the same way
	 * to hand the slot back to the sender. Firmware results, where a
	 * command has any, are returned in the reply payload of GET.
	 */
	 ...
	 */
	if (id == SSPM_CI_SET_ID) {
		if (sspm_ci_ack_matches(reply_hdr, expect_hdr)) {
			WRITE_ONCE(_hw_ack, 1);
		} else {
			WRITE_ONCE(_stale_acks, _stale_acks + 1);
			dev_dbg(__dev, "stale SET ack dropped: reply hdr %#x, in flight %#x (%u total)\n",
				reply_hdr, expect_hdr, _stale_acks);
		}
	}
...
static int sspm_ci_probe(struct platform_device *pdev)
{
	...
	if (!res)
		return -ENODEV;
	if (resource_size(res) < SSPM_MBOX_MMIO_SPAN)
		return dev_err_probe(dev, -EINVAL,
				     "mailbox window too small (%pa, need %#x)\n",
				     &res->end, SSPM_MBOX_MMIO_SPAN);
	base = devm_ioremap(dev, res->start, resource_size(res));
	if (!base)
		return -ENOMEM;

	/*
	 * The mailbox state is a single instance by design (one SSPM per
	 * SoC); refuse a second matching device instead of letting it take
	 * over the mapping while the first one's interrupts are live.
	 */
	mutex_lock(&_hw_rsc);
	if (sspm_mbox_iomap) {
		mutex_unlock(&_hw_rsc);
		return dev_err_probe(dev, -EBUSY,
				     "SSPM mailbox already bound to %s\n",
				     dev_name(__dev));
	}
