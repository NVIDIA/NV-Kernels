...
int mtk_build_eint(struct mtk_pinctrl *hw, struct platform_device *pdev)
{
	...
	hw->eint->event_pins = hw->soc->eint_event_unmask;
	hw->eint->num_event_pins = hw->soc->num_eint_event_unmask;
...
int mtk_pinctrl_program_bias_by_gpio(unsigned int gpio, u32 pullup, u32 arg)
{
	...
	/*
	 * Pin numbers are zero-based per controller, so refuse rather than
	 * guess if more than one registered instance covers @gpio.
	 */
	...
	if (!owner) {
		/* No controller yet is a probe-order problem, not a bad pin. */
		err = list_empty(&mtk_pinctrl_instances) ? -EPROBE_DEFER : -ENODEV;
		goto out;
	}
	...
	mutex_lock(&owner->pinconf_lock);
	err = owner->soc->bias_set_combo(owner, desc, int_pullup, int_arg);
	mutex_unlock(&owner->pinconf_lock);
