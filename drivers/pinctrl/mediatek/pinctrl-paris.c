			   ...
			   enum pin_config_param param, u32 arg)
{
	...
	case PIN_CONFIG_BIAS_BUS_HOLD:
		if (!hw->soc->bias_set_combo)
			break;
		/*
		 * Generic bias-bus-hold carries no argument (parsed as 0,
		 * which would read as MTK_DISABLE). Presence of the property
		 * means enable, so always request MTK_ENABLE here.
		 */
