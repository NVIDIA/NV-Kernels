...
static int efx_cxl_map_region(void *data)
{
	...
	struct range *cxl_pio_range = &cxl->region;
...
static void efx_cxl_unmap_region(void *data)
{
	...
	probe_data->cxl->ctpio_cxl = NULL;
}

void efx_cxl_fini(struct efx_probe_data *probe_data)
{
	if (probe_data->cxl_pio_initialised)
		devm_release_action(&probe_data->efx.pci_dev->dev,
				    efx_cxl_unmap_region, probe_data);
}
...
int efx_cxl_init(struct efx_probe_data *probe_data)
{
	...
	cxl->cxlmd = devm_cxl_probe_mem(&cxl->cxlds, &cxl->region);
	if (IS_ERR(cxl->cxlmd)) {
	...
	rc = efx_cxl_map_region(probe_data);
	if (rc)
		return rc;

	return devm_add_action_or_reset(&pci_dev->dev,
					efx_cxl_unmap_region, probe_data);
}
