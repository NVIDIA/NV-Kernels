...
void xhci_mtk_release_hw(struct xhci_hcd *xhci, bool hw_accessible)
{
	...
	/* Avoid IPPC and PHY accesses when the power state is unconfirmed. */
	if (hw_accessible) {
...
int xhci_mtk_get_resources(struct platform_device *pdev, struct xhci_hcd *xhci)
{
	...
	if (phy_type != PHY_HWIP_SPHY3 && phy_type != PHY_HWIP_SPHY4)
		return dev_err_probe(dev, -EINVAL,
				     "unsupported mtk-phy-type: %u\n", phy_type);
