...
static int mt8901_afe_disable_main_clock(struct mtk_sdw *mst)
{
	...
	/* Transactional: stop at the first failure, re-enable what was released. */
...
err_hopping:
	mt8901_afe_enable_top_cg(mst, AUD_CG_AUDIO_HOPPING_CK);
err_f26m:
	mt8901_afe_enable_top_cg(mst, AUD_CG_AUDIO_F26M_CK);
err_afe_on:
	mt8901_afe_enable_afe_on(mst);
...
err_vadsp:
	mt8901_afe_clk_mux_req(dev, AFE_CLK_VLP_CKSYS_TOP_VADSP_SEL, true);
err_f26m:
	mt8901_afe_enable_top_cg(mst, AUD_CG_AUDIO_F26M_CK);
...
err_ret:
	while (i-- > 0)
		mt8901_afe_clk_mux_req(dev, clk_array[i], true);
	return ret;
}
...
static int mt8901_sdw_disable_link_clock(struct mtk_sdw *mst, int link)
{
	...
	/*
	 * Transactional: stop at the first failure and re-enable what was
	 * already released, so the caller always sees either a fully clocked
	 * or a fully unclocked link.
	 */
	...
	if (sdw_link->active_clk_enabled) {
		...
		if (ret) {
			dev_err(mst->dev,
				"failed to disable active clock, ret %d\n", ret);
			goto err_main;
		}
		sdw_link->active_clk_enabled = false;
	}
...
err_active:
	if (!sdw_link->active_clk_enabled &&
	    !mt8901_afe_enable_sdw_active_clock(mst))
		sdw_link->active_clk_enabled = true;
err_main:
	mt8901_afe_enable_main_clock(mst);
err_sdw1:
	if (link != MTK_SDW_CONTROLLER_0)
		mt8901_afe_enable_top_cg(mst, AUD_CG_SOUNDWIRE1);
err_sdw0:
	mt8901_afe_enable_top_cg(mst, AUD_CG_SOUNDWIRE0);
err_apll2:
	mt8901_afe_enable_top_cg(mst, AUD_CG_APLL2_CK);
