...
static int mtk_sdw_read_prop(struct sdw_bus *bus)
{
	...
	/*
	 * sdw_bus_master_add() derives the bus data-rate limits from these
	 * right after read_prop returns, so the fallbacks must be in place
	 * here rather than after the master is added.
	 */
 ...
 */
/*
 * Reference driver restart choreography: soft-reset
...
int mtk_sdw_core_bus_reset(struct mtk_sdw_core *core)
{
	...
	/*
	 * Hold the bus message lock so the reset cannot overlap an
	 * sdw_transfer() issued by a peripheral driver on another CPU.
	 */
	...
	/*
	 * SOFT_RST clears the per-device slave interrupt masks. Callers that
	 * quiesced interrupts around the reset get them back from their
	 * enable_irq(true); the status work cannot use that helper (it would
	 * cancel itself), so restore the masks here when interrupts are live.
	 */
 ...
 * the attach is lost and the peripheral stays UNATTACHED forever. The
 * reference driver never hits this (its codec stack does not reset after
 * enumeration).
 */
...
irqreturn_t mtk_sdw_core_irq(int irq, void *data)
{
	...
	if (intstat & MCP_INT_LINE_ERRORS) {
		/*
		 * Isolated clashes or parity errors at bus power transitions
		 * are harmless. A persistent line error condition (floating
		 * pads on a link with no peripherals, or contention on a
		 * marginal link) refires the interrupt as fast as it is
		 * cleared and storms the CPU, to the point of starving the
		 * rest of the system: mask the line error interrupts that are
		 * firing on this link when a rapid burst is detected. The
		 * peripherals stay attached; their register traffic reports
		 * its own errors.
		 */
		...
		if (++core->clash_burst >= 32) {
			u32 storm = intstat & MCP_INT_LINE_ERRORS;

			mtk_sdw_update_intmask(core, storm, 0);
			dev_err(core->dev,
				"[%u]line error interrupt storm: masking 0x%x IntStatus:0x%x\n",
				bus->link_id, storm, intstat);
			core->clash_burst = 0;
		}
	}
	...
	if (intstat & MCP_INT_PARITY_ERR)
		dev_err_ratelimited(core->dev, "[%u]parity error 0x%x\n",
				    bus->link_id, intstat);
	...
	dev_info_ratelimited(core->dev, "[%u]ClearStatus 0x%x, IntStatus 0x%x\n",
