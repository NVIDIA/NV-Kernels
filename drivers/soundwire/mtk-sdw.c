...
static int mtk_sdw_parse_dp_allocation(struct mtk_sdw *mst, int idx)
{
	...
	/*
	 * { source_base, source_num, sink_base, sink_num }: every DP the
	 * firmware hands out indexes dp_pdi_mask[] and names a DP widget,
	 * so both ranges must fit in the MTK_SDW_MAX_DP ports of the
	 * manager. Check base first so the count test cannot underflow.
	 */
	if (dp[0] > MTK_SDW_MAX_DP || dp[1] > MTK_SDW_MAX_DP - dp[0] ||
	    dp[2] > MTK_SDW_MAX_DP || dp[3] > MTK_SDW_MAX_DP - dp[2])
		return dev_err_probe(mst->dev, -EINVAL,
				     "link %d: DP allocation src %u+%u sink %u+%u exceeds %u ports\n",
				     idx, dp[0], dp[1], dp[2], dp[3],
				     MTK_SDW_MAX_DP);
...
static void mtk_sdw_dai_release_params(struct mtk_sdw *mst,
				       struct mtk_sdw_dai *mdai)
{
	int i;
				 ...
				 struct snd_soc_dai *dai)
{
	...
	/* port_num is inside MTK_SDW_MAX_DP: validated when the DP allocation was parsed */
	pdis = mst->dp_pdi_mask[mdai->port_num];
	...
	if (tmp != pdi_count) {
		dev_err(dai->dev, "DP%u pdi mask 0x%lx doesn't match channel %u\n",
			mdai->port_num, pdis, channels);
		return -EINVAL;
	}

	/*
	 * ALSA allows HW_PARAMS again from SETUP or PREPARED without an
	 * intervening HW_FREE: drop the previous call's group sync
	 * reference and PDI state so nothing leaks or is released twice.
	 */
...
static int mtk_sdw_probe_controller(struct platform_device *pdev)
{
	...
	for (i = 0; i < mst->num_links; i++) {
		...
		/*
		 * The line is shared and MCP_INTMASK holds whatever POST left,
		 * so the handler can run as soon as it is registered: the lock
		 * and work items it uses must exist first.
		 */
