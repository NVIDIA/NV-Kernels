...
{
	return vdev->cxl && cxl_reset_capable(vdev->pdev);
}
 ...
 *
 * reinit_comp_regs() re-reads hardware into comp_reg_virt[] after reset.
 * The reset clears the real HDM decoder, which cxl-core restores to its
 * host-physical committed state.  reinit_comp_regs() then sees those host
 * bases, not the guest-physical bases the device manager programmed in
 * shadow, and the CTRL LOCK bit reads back per hardware.  Snapshot
 * BASE_LOW/BASE_HIGH and the LOCK bit from shadow before reinit, then
 * write them back so the emulated decoder stays consistent with what the
 * guest configured.
 *
...
static void vfio_cxl_reinit_hdm_shadow(struct vfio_pci_cxl_state *cxl)
{
	...
	if (cxl->comp_reg_virt && saved_lo) {
		for (n = 0; n < count; n++) {
			u32 ctrl_off = CXL_HDM_DECODER0_CTRL_OFFSET(n);
			__le32 *ctrl_reg = hdm_reg_ptr(cxl, ctrl_off);
			u32 ctrl;
			...
			*hdm_reg_ptr(cxl,
			 ...
			 * post-reset hardware state that reinit_comp_regs()
			 * just snapshotted, so leave those alone.
			 */
			ctrl = le32_to_cpu(*ctrl_reg);
			ctrl |= le32_to_cpu(saved_ctrl[n]) &
				CXL_HDM_DECODER0_CTRL_LOCK;
			*ctrl_reg = cpu_to_le32(ctrl);
		}
	}
