				      ...
				      size_t granule, bool leaf)
{
	...
	if (arm_smmu_inv_range_too_big(smmu_domain->smmu, size, granule)) {
		...
		if (smmu_domain->nesting_parent) {
			/*
			 * When the S2 domain changes all the nested S1 ASIDs
			 * have to be flushed too.
			 */
			cmd.opcode = CMDQ_OP_TLBI_NH_ALL;
			arm_smmu_cmdq_issue_cmd_with_sync(smmu_domain->smmu,
							  &cmd);
		}
	}
}
			     ...
			     struct iommufd_ctx *ictx, unsigned int viommu_type)
{
	...
	if (master->smmu->impl && master->smmu->impl->viommu_alloc)
		return master->smmu->impl->viommu_alloc(
