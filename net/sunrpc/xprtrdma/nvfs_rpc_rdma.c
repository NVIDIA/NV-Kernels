...
struct nvfs_dma_rw_ops *rpcrdma_nvfs_ops;

atomic_t rpcrdma_nvfs_shutdown = ATOMIC_INIT(1);

DEFINE_PER_CPU(long, rpcrdma_nvfs_n_ops);
...
int REGISTER_FUNC(struct nvfs_dma_rw_ops *ops)
{
	if (NVIDIA_FS_COMPAT_FT(ops)) {
		rpcrdma_nvfs_ops = ops;
		atomic_set(&rpcrdma_nvfs_shutdown, 0);
		return 0;
	}
	return -EOPNOTSUPP;
}
...
{
	(void)atomic_cmpxchg(&rpcrdma_nvfs_shutdown, 0, 1);
	do {
		msleep(NVFS_HOLD_TIME_MS);
	} while (nvfs_count_ops());
	rpcrdma_nvfs_ops = NULL;
}
