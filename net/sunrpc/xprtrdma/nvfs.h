...
extern struct nvfs_dma_rw_ops *rpcrdma_nvfs_ops;

extern atomic_t rpcrdma_nvfs_shutdown;

DECLARE_PER_CPU(long, rpcrdma_nvfs_n_ops);
...
static inline long nvfs_count_ops(void)
{
	...
	for_each_possible_cpu(i)
		sum += per_cpu(rpcrdma_nvfs_n_ops, i);
	return sum;
}

static inline bool nvfs_get_ops(void)
{
	if (rpcrdma_nvfs_ops && !atomic_read(&rpcrdma_nvfs_shutdown)) {
		this_cpu_inc(rpcrdma_nvfs_n_ops);
		return true;
	}
	return false;
}
...
{
	this_cpu_dec(rpcrdma_nvfs_n_ops);
}
