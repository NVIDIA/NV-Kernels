 ...
 *
 * Returns 0 on success, -EINVAL if @pullup or @arg is not one of the
 * values listed above or if more than one registered mtk_pinctrl owns
 * @gpio, -ENODEV if no registered mtk_pinctrl owns @gpio,
 * -EOPNOTSUPP if the owning controller has no bias_set_combo, or a
 * negative error code from the underlying bias operation.
 */
