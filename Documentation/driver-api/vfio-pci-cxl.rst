...
- A read/write VFIO device region (DPA) backed by the kernel-chosen host
  physical range for device memory. BOS does not advertise userspace mmap for
  this region.
...
``VFIO_CXL_CAP_FIRMWARE_COMMITTED``
    ...
    is live at device open; the VMM can use fd read/write without waiting for
    a guest COMMIT cycle.
...
BOS intentionally withholds ``VFIO_REGION_INFO_FLAG_MMAP`` for this region
until CPU-readable backing for CXL DPA PFNMAP can be proven safe. VMMs must
not assume the DPA region is mmap-capable; check the region flags and use fd
read/write when ``MMAP`` is absent.
...
HDM hardware state. The VMM can resume fd read/write without reopening the
device.
